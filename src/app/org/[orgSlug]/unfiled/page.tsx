...
import { getOrgBySlug, getUnfiledRankedOpportunities } from '@/lib/db';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Priorities missing valid time estimates in hours/week</p>
      </div>

      {unfiled.length === 0 ? (
        <div className="bg-slate-50 border border-emerald-200 rounded-xl p-8 text-center">
          <p className="text-emerald-600 text-lg font-medium">All priorities have valid time estimates</p>
          <p className="text-slate-500 mt-2 text-sm">Every priority has a valid hours/week value.</p>
        </div>
      ) : (
