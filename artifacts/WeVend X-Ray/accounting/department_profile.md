...
|---|---|---|---|
| Manual Phorge deal closing (multi-step, multi-layer navigation per terminal per deal) | ~1 hr/day = ~5 hrs/week | Kylie — daily | No automated deal closure; Phorge requires manual entry for every terminal in every deal |
| Commission calculation in Excel (per rep, per deal, per period) | Multi-hour per pay period — variable | Kylie — semi-monthly | No commission management platform; all calculations reference Schedule A manually |
| Manual forwarding of Clover payment confirmation emails | ~10 min/week (~2/day × 1 min each) | Nancy — daily | No Outlook automation rule or integration exists |
| Following up on delayed invoice/expense approvals from managers | Daily morning task — not quantifiable | Kylie (follow-up) + Nancy (oversight) — ongoing | No centralized approval workflow; approvals tracked manually by email. ~750 bills/year, 2–3 approval rounds each, each round up to 1–2 weeks |
| Duplicate customer setup in QBO and Clover | ~2–3 hrs/week (~480 invoices/year = ~10/week × 12–20 min each — hands-on keyboard time only, no email back-and-forth) | Nancy — daily | No system-generated PO importing to QBO; Clover requires separate manual setup. IT working on QBO solution; Clover not yet in scope |
| VISA statement reconciliation bottleneck | Preparation drag: 1–2 weeks/month per statement. QBD rec: 4 hrs/month (2 statements × 2 hrs) | Nancy / Kylie — monthly | No direct RBC VISA access; statements received 5–6 weeks after transactions occur; missing receipts discovered at month-end |
| Intercompany transaction reconciliation | Ongoing — not quantified | Nancy — ongoing | Manual entry in both QBD and QBO; accepted as necessary for now |
| Missing receipts at month-end credit card reconciliation | Absorbed into VISA statement prep drag above | Kylie — monthly | No unified expense platform; receipts submitted inconsistently by email |
