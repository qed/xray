-- Migration 014: Department lead experience (Phase 1 - dual-write)
--
-- Creates member_departments and priority_notes tables.
-- Backfills priorities.status from milestones.stage.
-- Updates apply_extraction to set status='proposed' (dual-write with milestones).
-- Adds bridge trigger: milestone stage changes sync to priorities.status.
-- Adds department_ids to invites for pre-assignment.
-- Fixes search_path on existing SECURITY DEFINER functions.

-- ============================================================
-- 1. Backup milestones for rollback safety
-- ============================================================
CREATE TABLE milestones_backup AS SELECT * FROM milestones;

-- ============================================================
-- 2. Create member_departments junction table
-- ============================================================
...
CREATE INDEX idx_member_departments_user ON member_departments(user_id);
...
-- ============================================================
-- 3. Create priority_notes table
-- ============================================================
CREATE TABLE priority_notes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_priority_notes_priority ON priority_notes(priority_id);
CREATE INDEX idx_priority_notes_user ON priority_notes(user_id);

ALTER TABLE priority_notes ENABLE ROW LEVEL SECURITY;

-- ============================================================
-- 4. Add department_ids to invites for pre-assignment
-- ============================================================
ALTER TABLE invites ADD COLUMN department_ids uuid[] DEFAULT '{}';

-- ============================================================
-- 5. Backfill priorities.status from milestones.stage
-- ============================================================

-- First clean up existing status values (all are 'Not started' or '')
UPDATE priorities SET status = 'not_started' WHERE status IN ('Not started', '');

-- Now backfill from milestones, deriving status entirely from milestone stage
UPDATE priorities p SET status = CASE m.stage
...
END
FROM milestones m
WHERE m.priority_id = p.id;

-- Orphaned priorities (no milestone) stay at 'not_started' from the cleanup above

-- ============================================================
-- 6. Bridge trigger: milestone stage changes sync to priorities.status
-- ============================================================
...
CREATE TRIGGER trg_bridge_milestone_to_status
...
-- ============================================================
-- 7. Fix search_path on existing SECURITY DEFINER functions
-- ============================================================
...
SET search_path = public
AS $$
  SELECT id FROM departments WHERE org_id IN (SELECT user_org_ids());
$$;

-- ============================================================
-- 8. Update apply_extraction to set status='proposed' (Phase 1: dual-write)
-- ============================================================
  ...
  ELSE
    ...
    IF v_profile->'teamMembers' IS NOT NULL AND jsonb_array_length(v_profile->'teamMembers') > 0 THEN
      ...
      FOR i IN 0..jsonb_array_length(v_profile->'teamMembers') - 1 LOOP
        ...
        INSERT INTO team_members (department_id, name, title, responsibilities, time_allocation)
        VALUES (
          v_dept_id,
          v_tm->>'name',
          COALESCE(v_tm->>'title', ''),
          COALESCE(v_tm->>'responsibilities', ''),
          COALESCE(v_tm->>'timeAllocation', '')
        )
